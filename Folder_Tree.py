...
class FolderTreeGenerator:
    ...
    def __init__(
        self, style="simple", icon_set="simple", max_depth=3, max_files_per_folder=None
    ):
    ...
    def _build_tree(
        ...
        include_categories: Optional[Set[str]],
        ...
        entries = [
            e
            for e in entries
            if self.should_include_file(e, include_categories, exclude_patterns)
        ]
    ...
    def save_text(
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            if header:
                ...
                f.write(
                    f"{project_name.upper()} Project Tree Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                )
    ...
    def save_png(self, output_path: str, font_size: int = 12) -> None:
        ...
        if not PIL_AVAILABLE:
            raise ImportError(
                "PIL (Pillow) is required for PNG output. Install with: pip install Pillow"
            )

        # Calculate image dimensions
        max_line_length = (
            max(len(line) for line in self.tree_lines) if self.tree_lines else 50
        )
    ...
    def save_pdf(self, output_path: str, page_size="A4") -> None:
        ...
        c.drawString(
            margin,
            y_position,
            f"Folders: {self.stats['folders']} | Files: {self.stats['files']} | Total Size: {self.format_size(self.stats['total_size'])}",
        )
...
Examples:
  ...
  %(prog)s /path/to/folder --max-files 5                      # Perfect for A4 documentation
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output file (extension determines format: .txt, .png, .pdf)",
    )
    parser.add_argument(
        "--style",
        choices=["simple", "professional", "artisanal"],
        default="simple",
    ...
    parser.add_argument(
        "--show-hidden", action="store_true", help="Include hidden files and folders"
    )
    parser.add_argument(
        "--no-sort-dirs", action="store_true", help="Don't sort directories first"
    )
    parser.add_argument(
        "--include-categories",
        nargs="+",
    ...
    parser.add_argument(
        "--formats",
        nargs="+",
        choices=["txt", "png", "pdf", "all"],
        default=None,
        help="Output formats to generate (txt, png, pdf, all). Use with --output-prefix",
    )
    parser.add_argument(
        "--output-prefix",
        default="folder_tree",
    ...
    try:
        ...
        if generator.stats["truncated_folders"] > 0:
            print(
                f"  Folders with truncated file lists: {generator.stats['truncated_folders']}"
            )
